...
const RegisterPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAuthToken = async () => {
            const domain = "dev-h8orlpa10u7tnfzz.us.auth0.com";
            const clientId = "HwESxixtawocpYnG6ha9yMaxHog9kiSS";
            const clientSecret = "WxG_xIIG3LuZEs8FykvJXiy5oZKKhlBLFCuUVwlgk7-eC-LUsxHNp7-NNfAb_B2a";

            if (!domain || !clientId || !clientSecret) {
                console.error("Check .env file for missing Auth0 credentials.");
                return;
            }
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.username || formData.username.length > 50) {
            newErrors.username = "Не більше 50 символів.";
        }

        if (!formData.fullName || formData.fullName.length > 500) {
            newErrors.fullName = "ФІО не більше 500 символів.";
        }

        if (!formData.password || formData.password.length < 8 || formData.password.length > 16) {
            newErrors.password = "Пароль від 8 до 16 символів.";
